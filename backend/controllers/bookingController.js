...
export const checkAvailabilityofCar = async (req, res) => {
    try {
        ...
        // fetch all available cars for the given locatioin
...
export const getOwnerBookings = async (req, res) => {
    try {
        ...
        const bookings = await Booking.find({ owner: req.user._id }).populate("car user").select({"-user.password"}.sort({cratedAt:-1}))
        res.json({success: true, bookings})
